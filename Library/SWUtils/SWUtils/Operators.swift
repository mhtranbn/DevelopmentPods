 ...
 */
public func -<Key, Value: Equatable>(left: Dictionary<Key, Value>, right: Array<Value>) -> Dictionary<Key, Value> {
    var result = Dictionary<Key, Value>()
    for key in left.keys {
        let val = left[key]!
...
/**
 ...
 - parameter right: List of values to remove
 */
public func -=<Key, Value: Equatable>(left: inout Dictionary<Key, Value>, right: Array<Value>) {
    var keyToRemove = [Key]()
    for key in left.keys {
        let val = left[key]!
        if right.contains(val) {
